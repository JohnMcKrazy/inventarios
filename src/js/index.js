...
const resultEditionsContainer = selector(
  '[cards-container="results_editions"]'
);
...
const changeTheme = () => {
  ...
  selectorAll(`[theme-icon]`).forEach((icon) => {
    icon.getAttribute("theme-icon") === state
      ? icon.setAttribute("show", "false")
      : icon.setAttribute("show", "true");
  });
  pageBody.setAttribute(
    "color-scheme",
    pageBody.getAttribute("color-scheme") === "dark" ? "light" : "dark"
  );
};
...
searchBtn.addEventListener("click", (e) => {
  ...
  if (!searchText || searchText === "") {
    ...
    selector("[dialog-ref='description']").textContent =
      "Necesitas mínimo 3 caracteres para recibir respuestas de búsqueda";
    ...
    selector(
      '[count="items"]',
      selector("[submenu-btn='results_items']")
    ).textContent = itemsCount;
    selector(
      '[count="editions"]',
      selector("[submenu-btn='results_editions']")
    ).textContent = editionsCount;
  }
});
...
const uniqueArray = collectionsSorted.filter(
  (obj, index, self) =>
    index === self.findIndex((o) => o.edition === obj.edition)
);
